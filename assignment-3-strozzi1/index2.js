...
/* //teacher's workaround
var alltwits = document.getElementsByClassName('some-class');
var allElems = [];
for (var i = 0; i < alltwits.length; i++) {
  allElems.push(alltwits[i]);
}*/
  ...
  if(searchtext.value!==""){            //if there's text in search box upons search
    for(i=0; i<alltwits.length; i++){    //going through all twits
      if(!alltwits[i].innerText.includes(searchtext.value) ){  //if twit includes search text
        //alltwits[i].remove();                 //delete
        twitCont.removeChild(alltwits[i]);
      }
    }
  }
}
